...
swiss_wide <- covid19swiss %>% filter(canton != "FL", 
...
Now, that we have both the canton data (`swiss_wide`) and geometry (`swiss_map`), we will merge the two using the `gn_a1_code` field as key:

```{r}
swiss_df <- swiss_map %>% left_join(swiss_wide, by = "gn_a1_code")
```
